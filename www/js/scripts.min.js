...
// Create a new instance of Headhesive.js and pass in some options
    ...
    setInterval(function (){
        $('.header__span').toggleClass('glitch-header')
    }, 3000)
    var $root = $('html, body');
    ...
    $('.anchor, .subject__button, .about__button').click(function () {
        $root.animate({
    ...
    $('.form-group').click(function(){
        ...
        $(this).siblings('.form-group').children('input').removeClass('active');
    })
        var movementStrength = 45;
        var height = movementStrength / $(window).height();
        var width = movementStrength / $(window).width();
        $(".subject__wrapper").mousemove(function(e){
            var pageX = e.pageX - ($(window).width() / 2);
            var pageY = e.pageY - ($(window).height() / 2);
            var newvalueX = width * pageX * -1 + 129;
            var newvalueY = height * pageY * -1 + 129;
            $('.subject__wrapper').css("background-position", newvalueX+"px     "+newvalueY+"px");
        });
    ...
    $('#popup__toggle').click(function(){
        $('.cover-bg, .modal').fadeToggle(300)
    })
    $('.modal__close').click(function(){
        $('.cover-bg, .modal').fadeOut(300)
    })
    ...
    $("#request-form").validator().on("submit", function (event) {
        if (event.isDefaultPrevented()) {
            // handle the invalid form...
            formError();
            submitMSG(false, "Перевірте, будь ласка, коректність введених даних");
        } else {
    ...
    function submitForm(){
        ...
        var email = $("#email").val();
        var phone = $("#phone").val();
        var check = $('.check.active').val();

        $.ajax({
            type: "POST",
            url: "form-process.php",
            data: "name=" + name + "&email=" + email + "&phone=" + phone + "&check=" + check,
    ...
    function formSuccess(){
        ...
        submitMSG(true, "Заявку прийнято!")
    }

    function formError(){
        $("#request-form").removeClass().addClass('shake animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
            $(this).removeClass();
        });
    }
    ...
    $(window).scroll(function() {
        if ($(this).scrollTop() > 4400) { //use `this`, not `document`
            $('#popup__toggle').fadeOut();
        } else if($(this).scrollTop() < 4400) {
            $('#popup__toggle').fadeIn();
        }
    });
    $('.white-preload, .yellow-preload').addClass('loaded')
    setTimeout(function(){
        $('.preloader__holder').remove()
    }, 3000)
    ...
    function makeTimer() {
        ...
        var endTime = new Date("10 September 2018 00:00:00 GMT+02:00");
